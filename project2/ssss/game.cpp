...
private:
    ...
    void setupTextures()
    {
        ...
        if (!carTexture.loadFromFile("satana2.png"))
        {
            // Handle error loading the texture
            std::exit(1);
        }
        ...
        if (!obstacleTextures[0].loadFromFile("victim.png") || !obstacleTextures[1].loadFromFile("victim1.png"))
        {
    ...
    void setupAudio()
    {
        if (!backgroundMusic.openFromFile("polish cow.ogg"))
        {
    ...
    void spawnObstacle()
    {
        int obstacleIndex = rand() % obstacleTextures.size();
        obstacles.emplace_back(obstacleTextures[obstacleIndex], BORDER_RIGHT - BORDER_LEFT, BORDER_LEFT, CAR_SPEED);
    }
};
